...
export const IpDetailModal: React.FC<IpDetailModalProps> = ({
  ...
  const titleNode = (
    <div className="flex items-center gap-2 min-w-0">
      <span className="font-mono font-bold text-slate-900 dark:text-white truncate">{ip}</span>
      <button
      ...
      <span className="text-xs font-normal text-slate-500 dark:text-slate-400 truncate hidden sm:inline">• {data?.countryName || "Đang tra cứu..."}</span>
    </div>
  );
  ...
  return (
    <ResponsiveModal open={isOpen} onClose={onClose} maxWidth="lg" title={titleNode} icon={<span className="text-xl">{data?.flagEmoji || "🌐"}</span>} footer={footerNode}>
