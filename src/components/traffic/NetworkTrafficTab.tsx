...
export const NetworkTrafficTab: React.FC = () => {
  ...
  const handleBlockIpDirect = async (ip: string) => {
    try {
      ...
      setBlockedFeedback(`Lỗi khi chặn IP ${ip}: ${e.message}`);
      setTimeout(() => setBlockedFeedback(null), 4000);
    }
  };
  ...
  return (
    <div className="space-y-4 sm:space-y-5 animate-in fade-in duration-200">
      ...
      {/* Main View: Responsive Mobile Cards (sm:hidden) and Desktop Table/Grid (hidden sm:block) */}
      <div className="block sm:hidden space-y-3">
        {loading && connections.length === 0 ? (
          <div className="p-8 text-center bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-3">
            ...
            <p className="text-xs text-slate-500 font-medium">Đang nạp danh sách kết nối mạng...</p>
          </div>
          ...
          filteredConnections.map((conn) => {
            ...
            return (
              <div
                key={conn.id}
                className={`bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-3 transition-all ${
              ...
              >
                ...
                <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 font-mono text-[11px] space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="text-slate-400 font-sans">IP Đích:</span>
                    <span className="font-bold text-slate-900 dark:text-white">
                      {conn.remoteAddress}:{conn.remotePort}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-slate-400 font-sans">IP Nguồn:</span>
                ...
                <div className="flex items-center justify-between pt-1">
                  ...
                  <div className="flex items-center space-x-1.5">
                    ...
                    {!conn.geo.isPrivate && (
                      <button
                        onClick={() => handleBlockIpDirect(conn.remoteAddress)}
      ...
      {/* Desktop Table or Grid (hidden sm:block) */}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-2 sm:px-3 font-mono hidden md:table-cell whitespace-nowrap">
                        <span className="text-slate-600 dark:text-slate-300">{conn.localAddress}</span>
                        <span className="text-slate-400 text-[10px]">:{conn.localPort}</span>
                      </td>
                      ...
                      <td className="py-3 px-3 font-mono font-bold whitespace-nowrap">
                        <button
                          onClick={() => setSelectedIpForModal(conn.remoteAddress)}
                          className="hover:underline text-slate-900 dark:text-white flex items-center space-x-1"
                        >
                          <span>{conn.remoteAddress}</span>
                          <span className="text-slate-400 text-[10px] font-normal">:{conn.remotePort}</span>
                        </button>
                      </td>
                      ...
                      <td className="py-3 px-3 whitespace-nowrap">
                        <div className="flex items-center space-x-1.5">
                          <span className="text-base shrink-0">{conn.geo.flagEmoji}</span>
                      ...
                      <td className="py-3 px-3 text-slate-600 dark:text-slate-300 hidden sm:table-cell max-w-[180px] truncate" title={conn.geo.isp}>
                        <div className="truncate font-medium">{conn.geo.isp}</div>
                        {conn.geo.asn && (
                          <div className="text-[10px] text-slate-400 font-mono truncate">{conn.geo.asn}</div>
                        )}
                      </td>
                      ...
                      {/* Protocol & State */}
                      <td className="py-3 px-2 sm:px-3 font-mono text-[10px] text-slate-500 dark:text-slate-400 hidden lg:table-cell whitespace-nowrap">
                        <span className="font-bold">{conn.protocol}</span> {conn.state}
                      </td>
                      ...
                      <td className="py-3 px-3 font-mono whitespace-nowrap">
                        <div className="font-bold text-slate-900 dark:text-white text-[11px]">
                          {isOutbound
                            ? `↑ ${formatSpeed(conn.txSpeedBytes)}`
                            : `↓ ${formatSpeed(conn.rxSpeedBytes)}`}
                        </div>
                        <div className="text-[10px] text-slate-400">
                          Tổng: {formatBytes(conn.totalTxBytes + conn.totalRxBytes)}
                        </div>
