...
export const NasTabBar: React.FC = () => {
  ...
  return (
    <div className="w-full bg-slate-100/90 dark:bg-slate-900/90 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800 px-4 sm:px-6 py-2 flex items-center justify-between gap-3 overflow-x-auto no-scrollbar transition-all">
      ...
      <div className="flex items-center gap-2 min-w-0 flex-1 overflow-x-auto py-0.5">
        ...
        {profiles.map((prof) => {
          ...
          return (
            <div
              ...
              className={`group px-3 py-1.5 rounded-xl text-xs font-bold transition-all shrink-0 cursor-pointer flex items-center gap-2 select-none border ${
                isActive
        ...
        <button
          ...
          className="px-2.5 py-1.5 rounded-xl bg-sky-600/10 hover:bg-sky-600 hover:text-white text-sky-600 dark:text-sky-400 text-xs font-bold border border-sky-500/30 transition-all shrink-0 flex items-center gap-1 shadow-xs"
