...
export const StorageManagerTab: React.FC = () => {
  ...
  const totalCapacity = volumes.reduce((acc, v) => acc + (v.totalBytes || 0), 0);
  const totalUsed = volumes.reduce((acc, v) => acc + (v.usedBytes || 0), 0);
    ...
    <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-200">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        ...
        <div className="bg-white dark:bg-slate-900 p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white mt-1 font-mono">
              {totalCapacity > 0 ? formatBytes(totalCapacity) : "6.98 TB"}
            </p>
        ...
        <div className="bg-white dark:bg-slate-900 p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-xl sm:text-2xl font-bold text-sky-600 dark:text-sky-400 mt-1 font-mono">
              {totalUsed > 0 ? formatBytes(totalUsed) : "3.27 TB"}
            </p>
            <p className="text-[11px] text-slate-400 mt-0.5">
              Còn trống {totalFree > 0 ? formatBytes(totalFree) : "3.71 TB"}
            </p>
          </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-4 sm:p-5 rounded-2xl sm:rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-[11px] text-slate-400 mt-0.5">
              {volumes.length} Volume đang hoạt động
            </p>
          </div>
      ...
      <div className="flex items-center justify-between">
        <h3 className="font-bold text-sm text-slate-900 dark:text-white flex items-center gap-2">
          ...
          Danh sách ổ đĩa & Tập lưu trữ (Volumes)
        </h3>
        <button
          onClick={loadStorage}
      ...
      <div className="space-y-4">
        {volumes.map((vol) => {
          const usedPct = vol.totalBytes > 0 ? Math.round((vol.usedBytes / vol.totalBytes) * 100) : 47;
          return (
            <div
              key={vol.id}
              className="bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-2xl sm:rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm space-y-4"
            >
              {/* Volume Header */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <div className="flex items-center space-x-3">
                  <div className="w-10 h-10 rounded-2xl bg-sky-500/10 text-sky-500 flex items-center justify-center font-bold text-xs shrink-0">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
                      {vol.path} • Hệ thống tệp: <span className="font-mono font-bold text-sky-500">{vol.fsType}</span>
                    </p>
                  </div>
                </div>
                ...
                <div className="w-full bg-slate-100 dark:bg-slate-800 h-2.5 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all duration-500 ${
                      usedPct > 85 ? "bg-rose-500" : usedPct > 70 ? "bg-amber-500" : "bg-sky-500"
                    }`}
                    style={{ width: `${Math.min(usedPct, 100)}%` }}
                  />
                </div>
                <div className="flex items-center justify-between text-[11px] text-slate-400">
                  <span>Còn trống: {formatBytes(vol.freeBytes)}</span>
                  <span>Tổng: {formatBytes(vol.totalBytes)}</span>
                </div>
              </div>
                ...
                <div className="pt-3 border-t border-slate-100 dark:border-slate-800 space-y-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5">
                    {vol.drives.map((d) => (
                      <div
                        key={d.slot}
                        className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800 flex items-start space-x-3"
                      >
