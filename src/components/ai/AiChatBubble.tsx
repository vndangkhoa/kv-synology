    ...
    <>
      ...
      <button
        ...
        className={`fixed z-40 flex items-center justify-center rounded-full shadow-2xl transition-all duration-300 active:scale-95 cursor-pointer
          ${
            isAiChatOpen
              ? "bottom-6 right-4 w-12 h-12 bg-[#1E1F20] text-white border border-white/10 rotate-90"
      ...
      >
        {isAiChatOpen ? (
          ...
          <div className="relative flex items-center justify-center">
            ...
            <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-emerald-400 rounded-full ring-2 ring-[#131314] animate-ping" />
          </div>
        )}
      </button>
      ...
      {isAiChatOpen && (
        <>
          ...
          <div
            className="fixed inset-0 z-30 bg-black/60 backdrop-blur-xs sm:bg-transparent sm:backdrop-blur-none sm:pointer-events-none transition-opacity duration-300"
            onClick={() => setAiChatOpen(false)}
          />

          <div
            className="fixed z-40 flex flex-col bg-[#1E1F20] border border-white/10 shadow-2xl overflow-hidden gemini-spring-sheet
            ...
            style={{
              ...
              boxShadow: "0 24px 48px -12px rgba(0, 0, 0, 0.8), 0 0 1px 1px rgba(255, 255, 255, 0.1)",
            }}
          >
            {/* Ambient Glow Aura when Generating */}
            {isGenerating && (
              <div
                className="absolute inset-0 pointer-events-none z-0 opacity-50 transition-opacity duration-500"
                style={{
                  background:
            ...
            <div className="flex items-center justify-between gap-2 px-4 py-3.5 border-b border-white/10 bg-[#1E1F20]/90 backdrop-blur-md shrink-0 z-10">
            ...
            <div className="flex-1 overflow-y-auto p-3.5 sm:p-4 space-y-3.5 bg-[#131314]/80 relative z-10">
              {messages.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center p-3 space-y-3">
                  <div className="w-14 h-14 rounded-full bg-gradient-to-tr from-[#4285F4]/20 via-[#9B51E0]/20 to-[#D9657B]/20 border border-white/10 text-[#8AB4F8] flex items-center justify-center shadow-lg animate-pulse">
                    <Sparkles className="w-7 h-7 text-[#8AB4F8]" />
                  </div>
                  <div className="max-w-[280px] space-y-1">
                    <h4 className="font-bold text-base text-white">Google Gemini for DSM</h4>
                  ...
                  <div className="grid grid-cols-1 gap-1.5 w-full max-w-[320px] pt-1">
                    {[
                      ...
                      <button
                        ...
                        className="text-left p-2.5 rounded-2xl bg-[#1E1F20] hover:bg-[#282A2C] border border-white/10 hover:border-[#4285F4]/40 text-xs font-medium text-slate-200 transition-all flex items-center gap-2.5 active:scale-95 shadow-sm"
                      >
                  ...
                  <div key={idx} className={`flex gap-2.5 ${msg.role === "user" ? "justify-end" : "justify-start"} gemini-stream-reveal`}>
                    ...
                    <div
                      className={`max-w-[82%] px-4 py-2.5 text-xs leading-relaxed relative group ${
                        msg.role === "user"
                          ? "bg-[#282A2C] text-white rounded-[20px_20px_4px_20px] border border-white/10 shadow-sm"
                          : "bg-transparent text-[#e3e3e3] rounded-[20px] pr-2"
                      }`}
                    >
                      ...
                      {msg.role === "assistant" && (
                        <button
                          ...
                          className="mt-1.5 p-1 rounded-lg bg-[#282A2C] border border-white/10 text-slate-400 hover:text-white transition-opacity shadow-xs inline-flex items-center gap-1 text-[10px]"
                        >
              ...
              {isGenerating && (
                <div className="flex gap-2.5 items-center pt-1 gemini-stream-reveal">
                  ...
                  <div className="p-2.5 px-4 rounded-full bg-[#1E1F20] border border-white/10 flex items-center gap-2">
                    <span className="text-[11px] font-bold text-[#8AB4F8]">Gemini đang suy nghĩ...</span>
            ...
            {messages.length > 0 && (
              <div className="px-3 py-1.5 bg-[#1E1F20] border-t border-white/5 flex items-center gap-1.5 overflow-x-auto scrollbar-none relative z-10">
                {["Giải thích SNMP", "Top tiến trình mạng", "Nhiệt độ CPU"].map((s) => (
                  <button
                    key={s}
                    onClick={() => handleSendMessage(s)}
                    className="px-3 py-1 rounded-full bg-[#282A2C] border border-white/10 text-[10.5px] font-medium text-slate-300 whitespace-nowrap hover:border-[#4285F4] hover:text-[#8AB4F8] transition-colors active:scale-95"
                  >
                    {s}
                  </button>
                ))}
            ...
            <div className="p-3 border-t border-white/10 bg-[#1E1F20] shrink-0 relative z-10">
              <div
                className={`flex items-center gap-2 p-1.5 pl-3 rounded-full bg-[#131314] border transition-all ${
                  isGenerating
                    ? "border-transparent ring-2 ring-purple-500/60 shadow-lg shadow-purple-500/20"
                    : "border-white/10 focus-within:border-[#4285F4]/60"
                }`}
              >
