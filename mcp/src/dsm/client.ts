...
import { DSMConnectionConfig, DSMSession, SystemInfo, SystemUtilization, DSMProcess, FileItem, ShareLink, DockerContainer, DownloadTask, StorageVolume, PackageItem, ServiceItem, TerminalInfo, FileServiceStatus, NotificationItem, AppNotifyItem, NotificationCategory, NotificationLevel } from "./types.js";
...
export class DSMClient {
  ...
  private rawPort: string = "";
  private rawHttps: boolean = true;
  ...
  private resolvedHttps: boolean | null = null;
  ...
  ensureConnected(): void {
    if (!this.session.isConnected) throw new Error("Not connected to DSM. Call dsm_login first.");
  }

  // --- Connection resolving ---
  private async ensureResolvedTarget(): Promise<{ host: string; port: number; isHttps: boolean }> {
    if (!this.config && !this.rawHost) throw new Error("No DSM host configured");
  ...
  setConnection(rawHost: string, rawPort: string | number, httpsFlag: boolean) {
  ...
  private async dsmRequest(
    ...
    return new Promise((resolve, reject) => {
      ...
      const req = client.request(reqOptions, (res) => {
        ...
        if (setCookie) {
          ...
          for (const sc of cookies) {
            ...
            if (mSid) this.session.sid = mSid[1];
            const mDid = sc.match(/did=([^;]+)/);
            if (mDid) this.session.did = mDid[1];
            const mToken = sc.match(/SynoToken=([^;]+)/);
            if (mToken) this.session.synoToken = mToken[1];
          }
        }
        // Capture syno token header
        if (res.headers["x-syno-token"]) this.session.synoToken = String(res.headers["x-syno-token"]);
        if (res.headers["x-syno-token2"]) this.session.synoToken = String(res.headers["x-syno-token2"]);
        ...
        res.on("end", () => {
          ...
          if (ct.includes("application/json") || bodyBuf.toString().startsWith("{")) {
            try {
              ...
              if (json?.data?.synotoken) this.session.synoToken = json.data.synotoken;
              if (json?.data?.did) this.session.did = json.data.did;
              resolve(json);
            } catch (e) {
      ...
      req.on("timeout", () => {
        req.destroy();
        const err: any = new Error("Connection timed out (30s)");
        err.code = "ETIMEDOUT";
        reject(err);
      });
  ...
  // --- Auth ---
  async login(config: DSMConnectionConfig, isDemo = false): Promise<DSMSession> {
    if (isDemo) {
      this.session = {
    ...
    try {
      const apiInfo: any = await this.dsmRequest("query.cgi", "GET", {
        api: "SYNO.API.Info",
        method: "query",
        version: "1",
        query: "all",
    ...
    for (const ver of authVersions) {
      try {
        ...
        const data: any = await this.dsmRequest("auth.cgi", "GET", params);
        if (data?.success && data?.data) {
          ...
          if (code === 400) throw new Error("Tài khoản hoặc mật khẩu không chính xác (400).");
          if (code === 401) throw new Error("Tài khoản đã bị khóa hoặc vô hiệu hóa (401).");
          if (code === 402) throw new Error("Quyền truy cập bị từ chối (402).");
          if (code === 403 || code === 406) {
            if (config.otp && config.otp.trim().length > 0) throw new Error("Mã OTP không chính xác hoặc đã hết hạn (403/406).");
            else throw new Error("Tài khoản yêu cầu mã OTP 6 chữ số (403). Vui lòng nhập otp_code.");
          }
          if (code === 404) {
            if (config.otp && config.otp.trim().length > 0) throw new Error("Mã OTP không đúng hoặc đã hết hạn (404).");
            continue;
          }
          if (code === 101) continue;
          if (data.error.message) lastError = data.error.message;
        }
      } catch (err: any) {
        if (err.message && (err.message.includes("OTP") || err.message.includes("mật khẩu") || err.message.includes("khóa") || err.message.includes("từ chối"))) throw err;
  ...
  // --- Generic entry.cgi ---
  async postEntry(api: string, method: string, version: number, extraParams: Record<string, string> = {}): Promise<any> {
    this.ensureConnected();
    if (this.session.isDemo) throw new Error("Demo mode: not connected to real DSM");
    const params: Record<string, string> = {
      api,
      method,
    ...
    const data: any = await this.dsmRequest("entry.cgi", "POST", params);
    return data;
  }

  // --- System ---
  async getSystemInfo(): Promise<SystemInfo> {
    if (this.session.isDemo || !this.session.isConnected) return mockSystemInfo;
    try {
      const data: any = await this.postEntry("SYNO.Core.System", "info", 1);
      if (data.success && data.data) {
        const d = data.data;
        let cpuStr = "";
        if (d.cpu_vendor || d.cpu_family || d.cpu_series) {
          ...
          cpuStr = parts.join(" ");
        } else cpuStr = "Intel Celeron J4125";
        ...
        if (typeof d.uptime === "number") parsedUptime = d.uptime;
        else if (typeof d.up_time === "string") {
          const [h, m, s] = d.up_time.split(":").map(Number);
          if (!isNaN(h) && !isNaN(m) && !isNaN(s)) parsedUptime = h * 3600 + m * 60 + s;
        }
  ...
  async getUtilization(): Promise<SystemUtilization> {
    if (this.session.isDemo || !this.session.isConnected) return getMockUtilization();
    try {
      const data: any = await this.postEntry("SYNO.Core.System.Utilization", "get", 1, { type: '"current"' });
      if (data.success && data.data) {
        const d = data.data;
  ...
  async getProcesses(): Promise<DSMProcess[]> {
    if (this.session.isDemo || !this.session.isConnected) return mockProcesses;
    try {
      const data: any = await this.postEntry("SYNO.Core.System.Process", "list", 1, { limit: "100", offset: "0", sort_by: '"cpu"', sort_direction: '"DESC"' });
  ...
  // --- Files ---
  async listShares(): Promise<FileItem[]> {
    if (this.session.isDemo || !this.session.isConnected) return mockFiles["/"] || [];
    try {
      const data: any = await this.postEntry("SYNO.FileStation.List", "list_share", 2, { additional: JSON.stringify(["perm", "time", "size", "owner", "real_path"]), limit: "1000", offset: "0", sort_by: '"name"', sort_direction: '"asc"' });
      if (data.success && Array.isArray(data.data?.shares)) {
        return data.data.shares.map((s: any) => ({ path: s.path, name: s.name, isdir: true, size: s.additional?.size || 0, mtime: (s.additional?.time?.mtime || 0) * 1000, owner: s.additional?.owner?.user || "admin", perm: s.additional?.perm?.posix ? `${s.additional.perm.posix}` : "0755", realPath: s.additional?.real_path || s.path }));
      }
    } catch {}
    return mockFiles["/"] || [];
  }
  ...
  async listFiles(folderPath: string): Promise<FileItem[]> {
    if (this.session.isDemo || !this.session.isConnected) return mockFiles[folderPath] || [];
    try {
      if (folderPath === "/" || folderPath === "") return this.listShares();
      const data: any = await this.postEntry("SYNO.FileStation.List", "list", 2, { folder_path: JSON.stringify(folderPath), additional: JSON.stringify(["perm", "time", "size", "owner", "real_path", "type"]), limit: "5000", offset: "0", sort_by: '"name"', sort_direction: '"asc"' });
      if (data.success && Array.isArray(data.data?.files)) {
        return data.data.files.map((f: any) => ({ path: f.path, name: f.name, isdir: !!f.isdir, size: f.additional?.size || 0, mtime: (f.additional?.time?.mtime || 0) * 1000, owner: f.additional?.owner?.user || "admin", perm: f.additional?.perm?.posix ? `${f.additional.perm.posix}` : "0644", realPath: f.additional?.real_path || f.path }));
      }
    } catch {}
  ...
  async createFolder(folderPath: string, name: string): Promise<boolean> {
    ...
    const data: any = await this.postEntry("SYNO.FileStation.CreateFolder", "create", 2, { folder_path: JSON.stringify(folderPath), name: JSON.stringify(name), force_parent: "false" });
    return !!data.success;
  }

  async deleteFile(filePath: string): Promise<boolean> {
    if (this.session.isDemo || !this.session.isConnected) return true;
    const data: any = await this.postEntry("SYNO.FileStation.Delete", "start", 2, { path: JSON.stringify([filePath]), accurate_progress: "true" });
    return !!data.success;
  }

  async renameFile(filePath: string, newName: string): Promise<boolean> {
    if (this.session.isDemo || !this.session.isConnected) return true;
    const data: any = await this.postEntry("SYNO.FileStation.Rename", "rename", 2, { path: JSON.stringify(filePath), name: JSON.stringify(newName) });
    return !!data.success;
  }

  async getFileContent(filePath: string): Promise<Buffer> {
    this.ensureConnected();
    if (this.session.isDemo) return Buffer.from("# Demo file content\nname: dsm-service\n");
    const target = await this.ensureResolvedTarget();
    const isHttps = target.isHttps;
    const host = target.host;
    const port = target.port;
    ...
    return new Promise<Buffer>((resolve, reject) => {
      const client = isHttps ? https : http;
      const req = client.request({ hostname: host, port, path: fullPath, method: "GET", headers: { Cookie: `id=${this.session.sid}`, "X-SYNO-TOKEN": this.session.synoToken || "" }, agent: isHttps ? httpsAgent : httpAgent, rejectUnauthorized: false } as any, (res) => {
        const chunks: Buffer[] = [];
        res.on("data", (c) => chunks.push(Buffer.isBuffer(c) ? c : Buffer.from(c)));
  ...
  // --- Upload (multipart) ---
  async uploadFile(folderPath: string, fileName: string, content: Buffer): Promise<boolean> {
    if (this.session.isDemo || !this.session.isConnected) return true;
    const target = await this.ensureResolvedTarget();
    const isHttps = target.isHttps;
    const host = target.host;
  ...
  // --- Share ---
  async createShareLink(path: string, password?: string, expireDate?: string): Promise<ShareLink> {
    if (this.session.isDemo || !this.session.isConnected) return { id: "demo123", url: `https://demo.quickconnect.to/sharing/demo123`, path, name: path.split("/").pop() || "Shared", date_expired: expireDate || "2026-12-31", has_password: !!password };
    const extra: Record<string, string> = { path: JSON.stringify([path]) };
    if (password) extra.password = JSON.stringify(password);
    if (expireDate) extra.date_expired = JSON.stringify(expireDate);
    const data: any = await this.postEntry("SYNO.FileStation.Sharing", "create", 3, extra);
    if (data.success && Array.isArray(data.data?.links) && data.data.links.length > 0) {
      ...
      else if (!url.startsWith("http")) url = `${baseUrl}/sharing/${item.id}`;
  ...
  async listShareLinks(): Promise<ShareLink[]> {
    if (this.session.isDemo || !this.session.isConnected) return [{ id: "demo123", url: "https://demo.quickconnect.to/sharing/demo123", path: "/docker/docker-compose.yml", name: "docker-compose.yml", date_expired: "2026-12-31", has_password: false }];
    try {
      const data: any = await this.postEntry("SYNO.FileStation.Sharing", "list", 3, { offset: "0", limit: "100" });
      if (data.success && Array.isArray(data.data?.links)) {
        const target = await this.ensureResolvedTarget();
        const baseUrl = `${target.isHttps ? "https" : "http"}://${target.host}:${target.port}`;
        return data.data.links.map((l: any) => ({ id: l.id, url: l.url?.startsWith("http") ? l.url : `${baseUrl}${l.url || `/sharing/${l.id}`}`, path: l.path, name: l.path?.split("/").pop() || l.id, date_expired: l.date_expired || "", has_password: !!l.has_password }));
      }
    } catch {}
    return [];
  }
  ...
  async deleteShareLink(id: string): Promise<boolean> {
    if (this.session.isDemo || !this.session.isConnected) return true;
    const data: any = await this.postEntry("SYNO.FileStation.Sharing", "delete", 3, { id: JSON.stringify([id]) });
    return !!data.success;
  }

  // --- Docker ---
  async getDockerContainers(): Promise<DockerContainer[]> {
    if (this.session.isDemo || !this.session.isConnected) return mockDockerContainers;
    try {
      const data: any = await this.postEntry("SYNO.Docker.Container", "list", 1, { limit: "-1", offset: "0", type: '"all"' });
      let resourceMap: Record<string, { cpu: number; memory: string }> = {};
      try {
        const resData: any = await this.postEntry("SYNO.Docker.Container.Resource", "get", 1);
  ...
  async toggleDockerContainer(id: string, action: "start" | "stop" | "restart"): Promise<boolean> {
    if (this.session.isDemo || !this.session.isConnected) return true;
    const method = action === "start" ? "start" : action === "stop" ? "stop" : "restart";
    const data: any = await this.postEntry("SYNO.Docker.Container", method, 1, { name: JSON.stringify(id) });
    return !!data.success;
  }

  // --- Download Station ---
  async getDownloadTasks(): Promise<DownloadTask[]> {
    if (this.session.isDemo || !this.session.isConnected) return mockDownloadTasks;
    try {
      let data: any = await this.postEntry("SYNO.DownloadStation2.Task", "list", 2, { additional: JSON.stringify(["detail", "transfer"]), limit: "500", offset: "0" });
      if (!data.success) data = await this.postEntry("SYNO.DownloadStation.Task", "list", 1, { additional: JSON.stringify(["detail", "transfer"]) });
  ...
  async toggleDownloadTask(id: string, action: "pause" | "resume" | "delete"): Promise<boolean> {
    if (this.session.isDemo || !this.session.isConnected) return true;
    const method = action === "pause" ? "pause" : action === "resume" ? "resume" : "delete";
    let data: any = await this.postEntry("SYNO.DownloadStation2.Task", method, 2, { id: JSON.stringify([id]) });
    if (!data.success) data = await this.postEntry("SYNO.DownloadStation.Task", method, 1, { id });
    return !!data.success;
  }

  // --- Storage ---
  async getStorageVolumes(): Promise<StorageVolume[]> {
    if (this.session.isDemo || !this.session.isConnected) return mockStorageVolumes;
    try {
      const res: any = await this.postEntry("SYNO.Storage.CGI.Storage", "load_info", 1);
      if (res.success && Array.isArray(res.data?.volumes)) {
        const diskMap: Record<string, any> = {};
        if (Array.isArray(res.data?.disks)) for (const d of res.data.disks) diskMap[d.id || d.name] = d;
        return res.data.volumes.map((v: any) => {
          ...
          const attachedDisks = Array.isArray(v.disks) ? v.disks.map((dName: string, idx: number) => {
            const diskObj = diskMap[dName] || {};
            return { slot: idx + 1, model: diskObj.model || diskObj.vendor || `Ổ đĩa ${idx + 1}`, serial: diskObj.serial || "N/A", status: (diskObj.status === "normal" ? "normal" : "warning") as any, temp: Number(diskObj.temp || 38), size: Number(diskObj.size_total_byte || 4000000000000), health: diskObj.smart_status === "normal" ? "Bình thường" : "Cần kiểm tra" };
          }) : [];
  ...
  async getPackages(): Promise<PackageItem[]> {
    if (this.session.isDemo || !this.session.isConnected) return mockPackages;
    try {
      const data: any = await this.postEntry("SYNO.Core.Package", "list", 1, { additional: JSON.stringify(["description", "status", "maintainer", "version", "display_name"]) });
      if (data.success && Array.isArray(data.data?.packages)) {
        return data.data.packages.map((p: any) => ({ id: p.id, name: p.additional?.display_name || p.name || p.id, version: p.additional?.version || p.version || "1.0", status: p.additional?.status === "running" ? "running" : "stopped", description: p.additional?.description || "", maintainer: p.additional?.maintainer || "Synology Inc." }));
      }
    } catch {}
    return mockPackages;
  }

  async togglePackage(id: string, action: "start" | "stop"): Promise<boolean> {
    if (this.session.isDemo || !this.session.isConnected) return true;
    const data: any = await this.postEntry("SYNO.Core.Package.Control", action, 1, { id: JSON.stringify(id) });
    return !!data.success;
  }
  ...
  async getTerminalInfo(): Promise<TerminalInfo> {
    ...
    if (data.success && data.data) return { enable_ssh: !!data.data.enable_ssh, enable_telnet: !!data.data.enable_telnet, ssh_port: Number(data.data.ssh_port || 22), hostname: data.data.hostname };
    throw new Error(data.error?.message || "Không thể lấy thông tin Terminal");
  }

  async setTerminal(enableSsh: boolean, enableTelnet?: boolean, sshPort?: number): Promise<boolean> {
    if (this.session.isDemo || !this.session.isConnected) {
      const ssh = mockServices.find((s) => s.id === "ssh");
      const telnet = mockServices.find((s) => s.id === "telnet");
      if (ssh) { ssh.enabled = enableSsh; ssh.running = enableSsh; ssh.status = enableSsh ? "running" : "stopped"; if (sshPort) ssh.port = sshPort; }
      if (telnet && enableTelnet !== undefined) { telnet.enabled = enableTelnet; telnet.running = enableTelnet; telnet.status = enableTelnet ? "running" : "stopped"; }
      return true;
    }
    let current: any = {};
    try { const cur: any = await this.postEntry("SYNO.Core.Terminal", "get", 3); if (cur.success) current = cur.data; } catch {}
    const payload: Record<string, string> = { enable_ssh: String(enableSsh), enable_telnet: String(enableTelnet ?? current.enable_telnet ?? false), ssh_port: String(sshPort ?? current.ssh_port ?? 22) };
    const data: any = await this.postEntry("SYNO.Core.Terminal", "set", 3, payload);
    return !!data.success;
  }

  async toggleService(id: string, enabled: boolean): Promise<boolean> {
    if (this.session.isDemo || !this.session.isConnected) {
      const svc = mockServices.find((s) => s.id === id);
      if (!svc) return false;
      svc.enabled = enabled; svc.running = enabled; svc.status = enabled ? "running" : "stopped";
      return true;
    }
    try {
      if (id === "smb") {
        ...
        const d = cur.data;
        const data: any = await this.postEntry("SYNO.Core.FileServ.SMB", "set", 3, { enable_samba: String(enabled), workgroup: JSON.stringify(d.workgroup || "WORKGROUP"), disable_shadow_copy: String(!!d.disable_shadow_copy), smb_transfer_log_enable: String(!!d.smb_transfer_log_enable) });
        if (!data.success) { const data2: any = await this.postEntry("SYNO.Core.FileServ.SMB", "set", 3, { enable_samba: String(enabled), workgroup: d.workgroup || "WORKGROUP" } as any); return !!data2.success; }
        return !!data.success;
      }
      if (id === "afp") {
        const cur: any = await this.postEntry("SYNO.Core.FileServ.AFP", "get", 1);
        const d = cur.data || {};
        const data: any = await this.postEntry("SYNO.Core.FileServ.AFP", "set", 1, { enable_afp: String(enabled), afp_transfer_log_enable: String(!!d.afp_transfer_log_enable) });
        return !!data.success;
      }
      if (id === "nfs") {
        const cur: any = await this.postEntry("SYNO.Core.FileServ.NFS", "get", 2);
        const d = cur.data || {};
        const data: any = await this.postEntry("SYNO.Core.FileServ.NFS", "set", 2, { enable_nfs: String(enabled), enable_nfs_v4: String(!!d.enable_nfs_v4), enable_nfs_v4_1: String(!!d.enable_nfs_v4_1 || !!d.enable_nfs_v4), nfs_v4_domain: JSON.stringify(d.nfs_v4_domain || "") });
        if (!data.success) { const data2: any = await this.postEntry("SYNO.Core.FileServ.NFS", "set", 2, { enable_nfs: String(enabled) } as any); return !!data2.success; }
        return !!data.success;
      }
      if (id === "ftp") {
        const cur: any = await this.postEntry("SYNO.Core.FileServ.FTP", "get", 3);
        const d = cur.data || {};
        const data: any = await this.postEntry("SYNO.Core.FileServ.FTP", "set", 3, { enable_ftp: String(enabled), enable_ftps: String(!!d.enable_ftps), timeout: String(d.timeout || 300), portnum: String(d.portnum || 21), custom_port_range: JSON.stringify(d.custom_port_range || ""), use_ext_ip: String(!!d.use_ext_ip), enable_fxp: String(!!d.enable_fxp), enable_fips: String(!!d.enable_fips), enable_ascii: String(!!d.enable_ascii), utf8_mode: JSON.stringify(d.utf8_mode || "auto") });
        if (!data.success) { const data2: any = await this.postEntry("SYNO.Core.FileServ.FTP", "set", 3, { enable_ftp: String(enabled) } as any); return !!data2.success; }
        return !!data.success;
      }
      if (id === "sftp") {
        const cur: any = await this.postEntry("SYNO.Core.FileServ.FTP.SFTP", "get", 1);
        const d = cur.data || {};
        const data: any = await this.postEntry("SYNO.Core.FileServ.FTP.SFTP", "set", 1, { enable: String(enabled), sftp_portnum: String(d.sftp_portnum || d.portnum || 22), portnum: String(d.portnum || d.sftp_portnum || 22) });
        return !!data.success;
      }
      if (id === "ssh" || id === "telnet") {
      ...
      const pkg: any = await this.postEntry("SYNO.Core.Package.Control", enabled ? "start" : "stop", 1, { id: JSON.stringify(id) }).catch(() => null);
      if (pkg && pkg.success) return true;
      throw new Error(`Dịch vụ '${id}' không được hỗ trợ`);
    } catch (e: any) { throw new Error(e.message || `Không thể ${enabled ? "bật" : "tắt"} ${id}`); }
  }

  async getFileServiceStatus(): Promise<FileServiceStatus> {
  ...
  public async getNotificationStrings(): Promise<Record<string, { title: string; msg: string }>> {
    if (this.session.isDemo || !this.session.isConnected) return {};
  ...
  private inferCategory(className: string, title: string): NotificationCategory {
  ...
  private inferLevel(title: string, msg: string): NotificationLevel {
  ...
  public async getNotifications(): Promise<NotificationItem[]> {
    ...
    try {
      ...
      if (!rawItems.length) return JSON.parse(JSON.stringify(mockNotifications));

      const parsed: NotificationItem[] = rawItems.map((item: any, idx: number) => {
  ...
  public async getAppNotifications(): Promise<AppNotifyItem[]> {
    if (this.session.isDemo || !this.session.isConnected) {
  ...
  public async clearNotifications(): Promise<boolean> {
    ...
    try {
      // Primary
      let data: any = await this.postEntry("SYNO.Core.DSMNotify", "notify", 1, {
        action: '"apply"',
        clean: '"all"',
      }).catch(() => null);
  ...
  // --- Power ---
  async powerAction(method: "reboot" | "shutdown", force = true): Promise<boolean> {
    if (this.session.isDemo || !this.session.isConnected) return true;
    const data: any = await this.postEntry("SYNO.Core.System", method, 1, { force: String(force), local: "true" });
    return !!data.success;
  }

  // --- API Info ---
  async getApiInfo(): Promise<any> {
    const data: any = await this.dsmRequest("query.cgi", "GET", { api: "SYNO.API.Info", method: "query", version: "1", query: "all" });
    return data;
  }

  // --- Generic raw entry.cgi for advanced use ---
  async rawEntryCall(api: string, method: string, version: number, params: Record<string, string> = {}): Promise<any> {
    return this.postEntry(api, method, version, params);
  }

  // --- Search & other helpers ---
  async searchFiles(folderPath: string, pattern: string, recursive = true): Promise<any> {
    this.ensureConnected();
    if (this.session.isDemo) return { success: true, data: { files: [] } };
    // SYNO.FileStation.Search start + list
    const startData: any = await this.postEntry("SYNO.FileStation.Search", "start", 2, { folder_path: JSON.stringify(folderPath), pattern: JSON.stringify(pattern), recursive: String(recursive), search_content: "false", search_type: JSON.stringify("simple") });
    if (!startData.success || !startData.data?.taskid) return startData;
    const taskId = startData.data.taskid;
    // Poll briefly
    await new Promise((r) => setTimeout(r, 600));
    const listData: any = await this.postEntry("SYNO.FileStation.Search", "list", 2, { taskid: taskId, offset: "0", limit: "100", sort_by: JSON.stringify("name"), sort_direction: JSON.stringify("asc"), additional: JSON.stringify(["real_path", "size", "owner", "time", "perm", "type"]) });
  ...
  async getLogs(limit = 50): Promise<any> {
    this.ensureConnected();
    try {
      const data: any = await this.postEntry("SYNO.Core.SyslogClient.Log", "list", 1, { start: "0", limit: String(limit), target: "LOCAL", logtype: "system,netbackup", method: "list" } as any);
      return data;
    } catch (e: any) {
  ...
  // --- SSH Exec (MCP direct) ---
  async execSshCommand(opts: { host: string; port?: number; username: string; password?: string; privateKey?: string; command: string }): Promise<{ success: boolean; stdout?: string; stderr?: string; code?: number | null; signal?: string; error?: string }> {
    const { host, port, username, password, privateKey, command } = opts;
    if (!host || !username || !command) return { success: false, error: "host, username, command required" };
