...
server.tool(
  "dsm_login",
  "Authenticate to Synology DSM. Supports LAN IP, DDNS, or QuickConnect ID (e.g. 'your-id' or 'your-id.quickconnect.to'). Will resolve QuickConnect via global.quickconnect.to with relay fallback. Use demo=true for offline mock data.",
  {
...
server.tool(
  "dsm_logout",
  "Logout from DSM and clear session",
  {},
  async () => {
    dsmClient.logout();
...
server.tool(
  "dsm_status",
  "Get current DSM connection status and session info",
  {},
  async () => {
    const s = dsmClient.getSession();
...
server.tool(
  "dsm_get_system_info",
  "Get DSM system info (model, serial, DSM version, uptime, temperature, RAM, CPU). Mirrors SYNO.Core.System.info",
  {},
  async () => {
    try {
      ensureLogin();
...
server.tool(
  "dsm_get_system_utilization",
  "Get real-time system utilization (CPU%, memory%, network RX/TX, disk read/write). Mirrors SYNO.Core.System.Utilization.get",
  {},
  async () => {
    try {
      ensureLogin();
...
server.tool(
  "dsm_list_processes",
  "List running processes (pid, name, cpu%, memory, user, status). Mirrors SYNO.Core.System.Process.list",
  {
...
server.tool(
  "dsm_power_action",
  "Reboot or shutdown the Synology NAS. Requires admin privileges. Mirrors SYNO.Core.System.reboot/shutdown",
  {
...
server.tool(
  "dsm_list_files",
  "List files/folders in a DSM shared folder path. Use '/' for share roots. Mirrors SYNO.FileStation.List (list_share + list).",
  {
...
server.tool(
  "dsm_create_folder",
  "Create a new folder in DSM FileStation. Mirrors SYNO.FileStation.CreateFolder.create",
  {
...
server.tool(
  "dsm_delete_file",
  "Delete a file or folder (recursive). Mirrors SYNO.FileStation.Delete.start",
  {
...
server.tool(
  "dsm_rename_file",
  "Rename a file or folder. Mirrors SYNO.FileStation.Rename.rename",
  {
...
server.tool(
  "dsm_get_file_content",
  "Download and return file content as text (for code, csv, log, json, yaml, etc) or base64 for binary. Mirrors SYNO.FileStation.Download",
  {
...
server.tool(
  "dsm_upload_file",
  "Upload a file to DSM FileStation. Content is base64-encoded or plain text. Mirrors SYNO.FileStation.Upload",
  {
  ...
  async ({ folder_path, file_name, content_base64, content_text }) => {
    try {
      ensureLogin();
      if (!content_base64 && !content_text) return err("Provide either content_base64 or content_text");
...
server.tool(
  "dsm_search_files",
  "Search files by name pattern across a folder (recursive). Mirrors SYNO.FileStation.Search",
  {
  ...
  async ({ folder_path, pattern, recursive }) => {
    try {
      ensureLogin();
      const res: any = await dsmClient.searchFiles(folder_path, pattern, recursive ?? true);
      return ok(res);
    } catch (e: any) {
      return err(e.message);
    }
...
server.tool(
  "dsm_list_shares",
  "List DSM shared folders (top-level). Alias for dsm_list_files with path='/' but explicit",
  {},
  async () => {
    try {
      ensureLogin();
...
// ---------- SHARE LINKS ----------
server.tool(
  "dsm_share_create_link",
  "Create a public sharing link for a file/folder (FileStation Sharing). Mirrors SYNO.FileStation.Sharing.create",
  {
...
server.tool(
  "dsm_share_list_links",
  "List all FileStation sharing links. Mirrors SYNO.FileStation.Sharing.list",
  {},
  async () => {
    try {
      ensureLogin();
...
server.tool(
  "dsm_share_delete_link",
  "Delete a sharing link by ID. Mirrors SYNO.FileStation.Sharing.delete",
  {
...
// ---------- DOCKER ----------
server.tool(
  "dsm_docker_list_containers",
  "List Docker/Container Manager containers with CPU/memory usage. Mirrors SYNO.Docker.Container.list + Resource.get",
  {},
  async () => {
    try {
      ensureLogin();
...
server.tool(
  "dsm_docker_container_action",
  "Start/stop/restart a Docker container. Mirrors SYNO.Docker.Container.start/stop/restart",
  {
...
// ---------- DOWNLOAD STATION ----------
server.tool(
  "dsm_download_list_tasks",
  "List DownloadStation tasks (HTTP, BT, FTP). Mirrors SYNO.DownloadStation2.Task.list",
  {},
  async () => {
    try {
      ensureLogin();
...
server.tool(
  "dsm_download_create_task",
  "Create a DownloadStation task from URL (http/https, magnet, ftp). Mirrors SYNO.DownloadStation2.Task.create",
  {
...
server.tool(
  "dsm_download_task_action",
  "Pause/resume/delete a DownloadStation task. Mirrors SYNO.DownloadStation2.Task.pause/resume/delete",
  {
...
// ---------- STORAGE ----------
server.tool(
  "dsm_storage_list_volumes",
  "List Storage Manager volumes and attached disks. Mirrors SYNO.Storage.CGI.Storage.load_info",
  {},
  async () => {
    try {
      ensureLogin();
...
server.tool(
  "dsm_package_list",
  "List installed DSM packages (Package Center). Mirrors SYNO.Core.Package.list",
  {},
  async () => {
    try {
      ensureLogin();
...
server.tool(
  "dsm_package_action",
  "Start or stop a DSM package. Mirrors SYNO.Core.Package.Control.start/stop",
  {
...
server.tool(
  "dsm_list_services",
  "List all DSM services (File Sharing: SMB/CIFS, AFP, NFS, FTP, SFTP; System: SSH, Telnet; Network: rsync, WebDAV). Shows enabled/running status, port, and details. Mirrors compound SYNO.Core.FileServ.* + SYNO.Core.Terminal.",
  {
  ...
  async ({ category, enabled_only }) => {
    try {
      ...
      if (enabled_only) filtered = filtered.filter((s) => s.enabled);
...
server.tool(
  "dsm_get_service",
  "Get details of a single DSM service by ID (smb, afp, nfs, ftp, sftp, ssh, telnet, rsync, webdav). Mirrors SYNO.Core.FileServ.* get.",
  {
  ...
  async ({ id }) => {
    try {
      ...
      if (!svc) return err(`Service '${id}' not found`);
      return ok(svc);
    } catch (e: any) {
      return err(e.message);
    }
...
server.tool(
  "dsm_toggle_service",
  "Enable or disable a DSM service. Handles read-modify-write: fetches current config then sets enabled flag while preserving other settings (workgroup, domains, ports, etc). Also works for packages as fallback. Use dsm_list_services to find valid IDs.",
  {
...
server.tool(
  "dsm_enable_service",
  "Enable/start a DSM service (alias for dsm_toggle_service enabled=true). Quick shortcut.",
  {
    id: z.string().describe("Service ID: smb | afp | nfs | ftp | sftp | ssh | telnet | rsync | webdav"),
  },
  async ({ id }) => {
...
server.tool(
  "dsm_disable_service",
  "Disable/stop a DSM service (alias for dsm_toggle_service enabled=false). Quick shortcut.",
  {
    id: z.string().describe("Service ID: smb | afp | nfs | ftp | sftp | ssh | telnet | rsync | webdav"),
  },
  async ({ id }) => {
...
server.tool(
  ...
  "Get SSH/Telnet terminal status (enable_ssh, enable_telnet, ssh_port). Mirrors SYNO.Core.Terminal.get version 3.",
  {},
  async () => {
    try {
      ensureLogin();
...
server.tool(
  "dsm_set_terminal",
  "Enable/disable SSH/Telnet and set SSH port. Mirrors SYNO.Core.Terminal.set version 3. Requires read-modify-write if port omitted.",
  {
...
server.tool(
  ...
  "Get summarized file sharing services status (SMB, AFP, NFS, FTP, SFTP). Mirrors dsm_helper fileService() compound.",
  {},
  async () => {
    try {
      ensureLogin();
...
server.tool(
  "dsm_ssh_exec",
  "Execute a shell command on the NAS (or any host) via SSH (ssh2). Requires host, username, password, command. Port defaults to 22 or current DSM SSH port. Supports demo mock. QuickConnect IDs not supported for SSH — use LAN IP/DDNS.",
  {
...
// ---------- NOTIFICATIONS ----------
server.tool(
  "dsm_list_notifications",
  "List DSM system notifications (DSMNotify). Returns grouped by title, parsed messages via DSMNotify.Strings templating, with category/level/time. Mirrors SYNO.Core.DSMNotify notify + SYNO.Core.DSMNotify.Strings.",
  {
  ...
  async ({ category, level, unread_only, limit }) => {
    try {
      ...
      if (category && category !== "all") notifs = notifs.filter((n) => n.category === category);
      if (level && level !== "all") notifs = notifs.filter((n) => n.level === level);
      if (unread_only) notifs = notifs.filter((n) => !n.read);
...
server.tool(
  "dsm_get_app_notifications",
  "List DSM app notifications (App Center, Hyper Backup, Photos, etc). Mirrors SYNO.Core.AppNotify get.",
  {},
  async () => {
    try {
      ensureLogin();
...
server.tool(
  "dsm_clear_notifications",
  "Clear all DSM notifications (mark as cleaned). Mirrors SYNO.Core.DSMNotify notify action apply clean all. Demo clears mock.",
  {},
  async () => {
    try {
      ensureLogin();
...
server.tool(
  "dsm_mark_notifications_read",
  "Mark all notifications as read (sets lastRead/lastSeen to now). Mirrors DSMNotify load with now.",
  {},
  async () => {
    try {
      ensureLogin();
...
server.tool(
  "dsm_get_notification_strings",
  "Get DSMNotify string templates (title/msg) map for templating. Mirrors SYNO.Core.DSMNotify.Strings get lang enu.",
  {},
  async () => {
    try {
      ensureLogin();
...
server.tool(
  "dsm_get_api_info",
  "Query DSM API info (query.cgi?api=SYNO.API.Info&method=query). Lists all available APIs and versions. Useful for discovering endpoints.",
  {},
  async () => {
    try {
      ensureLogin();
      const info: any = await dsmClient.getApiInfo();
      return ok(info);
    } catch (e: any) {
      return err(e.message);
    }
...
server.tool(
  "dsm_raw_entry_call",
  "Raw SYNO.Entry.Request call to entry.cgi for any DSM API. Advanced: specify api, method, version, and extra params as JSON string. Mirrors Util.post('entry.cgi', data={api,method,version,...})",
  {
  ...
  async ({ api, method, version, params_json }) => {
    try {
      ...
      const res: any = await dsmClient.rawEntryCall(api, method, version, extra);
      return ok(res);
    } catch (e: any) {
      return err(e.message);
    }
...
server.tool(
  "dsm_get_logs",
  "Get DSM system logs (Log Center). Mirrors SYNO.Core.SyslogClient.Log.list",
  {
  ...
  async ({ limit }) => {
    try {
      ensureLogin();
      const logs: any = await dsmClient.getLogs(limit ?? 50);
      return ok(logs);
    } catch (e: any) {
      return err(e.message);
    }
